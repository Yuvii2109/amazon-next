...
def create_intent_bundle(payload: IntentRequest):
    ...
    system_instruction = (
        "You are an expert retail merchandiser for an online grocery and household store. "
        "We have a store promotion: if a user spends ₹1000, they get ₹100 off. "
        "Your goal is to fulfill the user's intent by curating UP TO 3 distinct "
        ...
        "- ONLY include items that are genuinely relevant to the user's stated intent.\n"
        "- Do NOT pad bundles with irrelevant or unrelated items just to hit a price target.\n"
        "- If the catalog items provided have NO meaningful relevance to the user's intent "
        "(e.g., user asks for electronics/hardware/sports equipment but catalog only has groceries), "
        "return an empty bundles array and set message to a helpful explanation of what the store "
        "carries and why the request cannot be fulfilled.\n"
        "- If the catalog has PARTIAL relevance (e.g., user asks for 'party supplies' and catalog has "
        "snacks/beverages but not decorations), create bundles with the relevant items you CAN find "
        "and note in message what is missing.\n"
        "- If you can suggest adjacent/related products as a softer fallback (e.g., user asks for "
        "'bat repair materials' but catalog has sports nutrition or fitness items), put those in extras "
        "with an explanatory message. Only do this if the alternatives are genuinely useful to the user.\n\n"
        "OTHER RULES:\n"
    ...
    prompt = (
        ...
        f"Relevant catalog items (pre-filtered by similarity): {json.dumps(retrieved_items, ensure_ascii=False)}\n\n"
        "Return a JSON object with exactly these keys: bundles, extras, message.\n"
        "bundles must be an array of 0-3 objects, each with: name (string), items (array of product objects with id, name, price, image_url), total_cost (number = sum of item prices in that bundle).\n"
        "extras must be an array of 0-10 product objects with id, name, price, and image_url.\n"
        "message must be either null (when bundles are successfully created with relevant items) "
        "or a short user-friendly string explaining why no/partial bundles were created.\n"
        "Bundle names must be exactly one of: 'Smart Saver', 'Popular Choice', 'Premium Selection'.\n\n"
        "IMPORTANT: If the provided catalog items are NOT relevant to the user's intent, "
        "return {\"bundles\": [], \"extras\": [], \"message\": \"<explanation>\"}. "
        "Do NOT force irrelevant products into bundles."
    )
