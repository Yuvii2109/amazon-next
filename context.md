...
- Added Speech-to-Text (STT) functionality using the Web Speech API. Created `src/hooks/use-speech-recognition.ts` hook wrapping `SpeechRecognition` / `webkitSpeechRecognition` with `en-IN` locale, continuous mode, and interim results. Wired into: (1) AI Intent Engine — mic button in the textarea that fills the input with spoken text, user clicks Generate manually; (2) Navbar search bar (desktop + mobile) — mic button triggers voice input that auto-searches via the existing debounce logic. Mic buttons show an animated pulse while listening and gracefully hide on unsupported browsers.
- Made the AI Intent Engine robust against irrelevant queries. Updated the Gemini system prompt with explicit relevance rules: if catalog items don't match the user's intent, return empty bundles with a `message` explanation. Supports a softer fallback where adjacent/related products appear in `extras` with an explanatory note. Added `message: str | None` field to `IntentResponse` model. Frontend now shows a friendly "No matching products found" amber banner when bundles are empty, renders fallback extras as "You might find these useful instead", and displays a blue partial-match note when bundles exist but coverage is incomplete.